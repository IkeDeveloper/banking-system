...
class SavingsAccount(BankAccount):
    def __init__(self, account_number, firstname, surname, telephoneno, email, address, interest_rate=0.02):
        super().__init__(account_number, firstname, surname, telephoneno, email, address)
        self.interest_rate = interest_rate  # Higher interest for savings
...
class BankingSystemManager:
    def __init__(self):
        ...
        self.id_gen = AccountIDGenerator("A") #create an instance of the ID generator for Bank Account
        
        self.savings_id_gen = AccountIDGenerator("B") #Create an instance of the ID generator for the SavingsAccount
    
    def run(self): #Call from main
        while True:
            self.display_menu()
    ...
    def display_menu(self):
        ...
        print("Q. Erase database table 'accounts'.")
        print("0. Exit")
        print("="*50)
    ...
    def printallaccounts(self): #This option is working
        """Print all accounts"""
        print("\n--- ALL BANK ACCOUNTS ---")
    ...
    def ministatement_menu(self): #Option 7 does not work when data loaded from database
    ...
    def createSavingsAccount(self): #Creates a savings bank account
        ...
        while True:
            ...
            new_id = self.savings_id_gen.next_id()
    ...
    def save_accounts_to_db(self):
        ...
        if not self.accounts:  # dictionary is empty
            print("Error: No accounts in memory to save.")
            return  # exit early
        ...
        if exists: #Print error if data with identical ID already exists it database
         ...
         # Loop through dictionary and insert
            for acc_id, acc_obj in self.accounts.items():
                ...
                (account_id, account_number, firstname, surname, telephoneno, email, balance) 
                VALUES (%s, %s, %s, %s, %s, %s, %s)"""
                values_account = (
                    acc_id,
                    acc_obj.account_number,
                    acc_obj.firstname,
                    acc_obj.surname,
                    acc_obj.telephoneno,
                    acc_obj.email,
                ...
                cursor.execute(sql_account, values_account)

            addr = acc_obj.address
            sql_address = """INSERT INTO addresses 
            (account_id, street_number, street_name, city, postcode, country) 
            VALUES (%s, %s, %s, %s, %s, %s)"""
            values_address = (
                acc_id,
                addr.street_number,
    ...
    def load_accounts_from_db(self):
        ...
        cursor.execute("""
            SELECT a.account_id, a.account_number, a.firstname, a.surname,
               a.telephoneno, a.email, a.balance,
               ad.street_number, ad.street_name, ad.city, ad.postcode, ad.country
            FROM accounts a
        ...
        for row in rows:
            ...
            account.balance = row["balance"]  # if balance is part of your model

            # Store in dictionary keyed by account_id
    ...
    def erasedatabase(self):
        ...
        print("SQL Database table 'accounts' deleted successfully.")
